...
def generate(audiofile,pdffile):
    ...
    # タスク2
    この音声データは、自動車販売店の商談時の録音です。salesとcustomerのどちらの発話かが分かるように、下記の形式で文字起こししてください。Timestampは発話が開始された時間を使ってください。
    salesとはcustomerに対して、車の紹介をしたり見積もりを作成したりする役割です。
